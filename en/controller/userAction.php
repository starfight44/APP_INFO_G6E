    ...
    if(isLoginOk()){
        ...
        if(isset($_POST['password'])) {
            if (password_verify($_POST['password'],$connectionDatas['password'])) {
                    ...
                    $warning_message = 'modifications successful, log in again!';
                    header('Location:index.php?action=userInformations');

            } else {
                $warning_message = 'Failed, incorrect password, try again.';
...
function userHistory(){
    if(isLoginOk()){
            ...
            if(count($donnees)==0){
                $resultList = '<strong><p>You have not done any test, go to the test section!</p></strong>';
            }
            else{
                ob_start();
                echo '<tr>
...
function printUserChat(){
    if(isLoginOk()){
        ...
        foreach($donnees as $elt) {
            echo '<h4';
            if($elt['account_type']=='manager'){echo ' id="manager">Manager';}else{echo ' id=user>You';}
            echo '</h4>
